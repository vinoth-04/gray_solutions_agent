...
from database.tools import check_slot, book_slot, next_available_slot, reschedule_slot, cancel_slot, transfer_to_human
...
async def run_bot(transport, call_sid: str = ""):
    ...
    # Injects the live call_sid into FunctionCallParams before delegating to
    # the real transfer_to_human tool so the Exotel Redirect API knows
    # which call to forward.
    async def transfer_to_human_with_sid(params: FunctionCallParams):
        ...
        params.call_sid = call_sid          # attach call_sid at runtime
    ...
    tools_schema = ToolsSchema(
        standard_tools=[
            ...
            reschedule_slot,
            cancel_slot,
            transfer_to_human_with_sid,   # ← wrapper with call_sid injection
        ]
    )
